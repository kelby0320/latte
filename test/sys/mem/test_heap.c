...
#define HEAP_SIZE                                                              \
    (LATTE_HEAP_MIN_BLOCK_SIZE * (1 << LATTE_HEAP_MAX_ORDER) *                 \
     LATTE_HEAP_LARGE_BLOCKS)
...
test_heap_malloc_mixed_allocations(void *state)
{
    ...
    void *ptr_900k =
        heap_malloc(heap, (1024 * 900)); /* order 8 block (1024k) */
    void *ptr_1800k =
        heap_malloc(heap, (1024 * 1800)); /* order 9 block (2048k) */
    void *ptr_3000k =
        heap_malloc(heap, (1024 * 3000)); /* order 10 block (4096k) */
