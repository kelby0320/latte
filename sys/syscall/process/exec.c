...
copy_user_argv(struct thread *current_thread, const char *const *user_argv)
{
    ...
    thread_copy_from_user(
        current_thread, (void *)user_argv, argv, MAX_ARGS - 1);
    ...
    for (argv_len = 0; argv_len < MAX_ARGS; argv_len++) {
        ...
        thread_copy_from_user(
            current_thread, (void *)argv[argv_len], arg, MAX_ARG_LEN - 1);
        argv[argv_len] = arg;
    }
...
do_execv(struct thread *current_thread)
{
    ...
    const char *const *user_argv =
        (const char *const *)thread_get_stack_item(current_thread, 1);
    const char *user_path =
        (const char *)thread_get_stack_item(current_thread, 2);

    char path[LATTE_MAX_PATH_LEN];
    memset(path, 0, sizeof(path));

    thread_copy_from_user(
        current_thread, (void *)user_path, path, sizeof(path) - 1);
    ...
    process_execv(
        current_thread->process, path, (const char *const *)argv, argv_len);
