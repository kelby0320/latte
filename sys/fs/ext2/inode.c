 ...
 */
static int
ext2_read_block_group_desc(
    struct ext2_block_group_descriptor **desc_out,
    struct ext2_private *fs_private, int block_group)
{
    ...
    struct ext2_block_group_descriptor *desc =
        kzalloc(sizeof(struct ext2_block_group_descriptor));
    if (!desc) {
        return -ENOMEM;
    }
    ...
    uint32_t inode_tbl, uint32_t inode_no)
{
    ...
    int inode_start = inode_tbl_start +
                      ((inode_no - 1) * fs_private->superblock.s_inode_size);
    ...
    int res = block_buffered_reader_read(
        fs_private->reader, (char *)inode, sizeof(struct ext2_inode));
    if (res < 0) {
        kfree(inode);
        return res;
    }
    ...
    char *out)
{
    ...
    return block_buffered_reader_read(
        fs_private->reader, out, fs_private->block_size);
}
    ...
    uint32_t inode_no)
{
    ...
    res = ext2_read_inode_from_tbl(
        &inode, fs_private, desc->bg_inode_table, inode_no);
    if (res < 0) {
        return res;
    }
